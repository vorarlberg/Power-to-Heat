...
| `PUMP_SCRIPT_CHECK` | Versionsüberwachung der Unterskripte | Prüft alle `300 s`, ob Heizkreispumpe `1.3.0`, Speicherladepumpe `1.2.7`, WW-Pumpe `1.1.1` und Lufttrockner `1.3.4` melden. |
...
- Fällt die Leistungsaufnahme trotz eingeschaltetem Shelly länger ab, wird die Tankmeldung gesetzt und Pushover über `pushover.0` nur einmal gesendet; automatische PV-Abschaltungen setzen diese Meldesperre nicht zurück. Nach dem Leeren kann `Betriebsmodus Ein` genutzt werden, damit das Skript die wieder vorhandene Leistungsaufnahme erkennt und die Meldung zurücksetzt.
